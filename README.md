...
| 选项 | 说明 |
| --- | --- |
| **① 远端 API** | 由第三方公共接口代为探测，浏览器直连，不需要本机后端；IPv4 / IPv6 都能检测，但拿不到延迟，且依赖第三方可用性。展开后可选择具体接口：`mcsrvstat.us`（IPv4 / IPv6 均支持）、`mcstatus.io`（备用，对部分 IPv6 目标支持有限）。 |
| **② 本地后端（默认）** | 使用本机运行的 Node 后端（`127.0.0.1:8787`），数据最完整（含延迟测量），探测全部在本机完成。**若连不上会自动切换到远端 API**，并在卡片下方说明切换原因，不影响查看。 |
| **③ 客户端访问** | 尝试在浏览器里直接完成 mcping。浏览器出于安全限制**无法建立原始 TCP 连接**，所以该项标记为「不可用」并注明原因；将来若接入 WebSocket/TCP 网关才可实现。 |
