...
`tools/build-config.js` 里的 `server.enabled` 始终为 `false`（提交进仓库、线上构建据此保持置灰）；
而本地 `server.js` 在提供 `/config.js` 时会**动态注入覆盖**：把 ② 设为启用并作为默认数据来源。
因此：

- 打开 <http://127.0.0.1:8787>（本机页面）→ 默认走「Imikufans后端」，可测延迟，数据最完整
- 线上 `status.yunmc.icu` / `status.swordsman.top` → ② 依旧置灰（备案中），默认走 ① 远端 API
- 本地后端若未运行，页面会自动回退到远端 API

若希望线上也启用，把 `tools/build-config.js` 里 `server.enabled` 改成 `true` 重新构建即可。
...
| 选项 | 说明 |
| --- | --- |
| **① 远端 API（默认）** | 由第三方公共接口代为探测，浏览器直连，不需要本机后端；IPv4 / IPv6 都能检测，但拿不到延迟，且依赖第三方可用性。选中后下方会出现第二个下拉框「选择探测接口」，可在 `mcsrvstat.us`（IPv4 / IPv6 均支持）与 `mcstatus.io`（备用，对部分 IPv6 目标支持有限）之间切换。 |
| **② Imikufans后端** | 在自有物理机上运行的 Node 服务，数据最完整（含延迟测量）。**本地页面已启用且为默认项**；**线上版本因合规要求（对外提供 Web 服务需完成 ICP 备案）仍置灰不可点击**，预计 **2027 年 1 月左右**完成备案后于 `ipv6.swordsman.top:8787` 对外开放。说明文字中含致谢：「感谢 shen 的大力支持！」 |
