...
function aggregateServerStatus(endpoints) {
  ...
  if (online === 0) return 'down'; // 全部掉线 → 红色
  return 'partial'; // 部分在线 → 黄色
}
...
async function runChecks(reason) {
  ...
  const servers = config.servers.map((server) => {
    ...
    return {
      ...
      onlineCount: endpoints.filter((e) => e.online).length,
  ...
  snapshot = {
    ...
    summary: {
      ...
      endpointsOnline: allEndpoints.filter((e) => e.online).length,
